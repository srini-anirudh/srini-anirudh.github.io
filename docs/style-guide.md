...
- The shared blog shell derives a Distill-inspired side contents rail from that inline table of contents on wide screens. Do not add article-specific fixed TOC rails: the shared rail measures the available gutter, appears only when it cannot overlap the article, highlights the current section, and stays hidden at narrower widths where the inline TOC remains available.
