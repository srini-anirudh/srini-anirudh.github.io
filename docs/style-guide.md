...
- The shared blog shell derives a Distill-inspired side contents rail from that inline table of contents after the inline TOC has scrolled past. At desktop widths of 1440px and above, the rail occupies one fixed 200px column at the left with a 32px gap; the shared shell contains wide tables, figures, and interactive panels inside the article canvas to its right. Do not add article-specific fixed TOC rails or viewport-centred exceptions to this contract. At narrower widths the rail stays hidden and the inline TOC remains available.
...
- For a named series, keep its cards contiguous and number every entry visibly within the card label; do not add a separate full-width series banner to the index. Add a stable category filter and repeat a compact linked previous/next navigator after each article mast.
